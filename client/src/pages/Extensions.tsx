...
import { Code2, DownloadCloud, PackageCheck, TerminalSquare } from "lucide-react";
import { caddy as caddyApi, jcode as jcodeApi, processes as processApi } from "../api";
...
export default function Extensions() {
  ...
  const [jcodeLoading, setJcodeLoading] = useState(true);
  const [jcodeInstalling, setJcodeInstalling] = useState(false);
    ...
    <div className="compact-page-stack">
      ...
      <section className="extension-status-card">
        ...
        <div className="grid gap-2 xl:grid-cols-2">
          <InsetPanel padding="sm" className="extension-card-row">
            ...
            {jcodeLoading ? (
              ...
              <div className="min-w-0 flex-1">
                <SubsectionTitle>JCode</SubsectionTitle>
                <SupportingCopy>
                  Status: {" "}
                  <StatusText tone={jcodeStatus?.installed ? "success" : "warning"}>
                    {jcodeStatus?.installed ? `Installed${jcodeStatus?.version ? ` (${jcodeStatus.version})` : ""}` : "Not installed"}
                  </StatusText>
                </SupportingCopy>
                <SupportingCopy>Use the JCode tab to control provider access, gateway, and pairing.</SupportingCopy>
              </div>
            )}
            {jcodeLoading ? (
              <Skeleton className="h-10 w-32" />
            ) : jcodeStatus?.installed ? (
              <Button as={Link} to="/dashboard/jcode" variant="primary">
                Open JCode
              </Button>
            ) : (
              <Button type="button" variant="outlineInfo" disabled={jcodeInstalling} onClick={installJcode}>
                <DownloadCloud size={16} />
                {jcodeInstalling ? "Installing..." : "Install JCode"}
              </Button>
            )}
          </InsetPanel>
        ...
        {!status.installed && Array.isArray(status.installCommands) && status.installCommands.length > 0 && (
          <InsetPanel className="mt-3 text-xs text-text-3">
            <p className="mb-1 text-text-2">Detected Caddy install command(s)</p>
